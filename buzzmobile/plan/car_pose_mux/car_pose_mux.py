...
def mux(car_state):
    if car_state == CarState.AUTO: return state['auto_car_pose']
    if car_state == CarState.MANUAL: return state['manual_car_pose']
...
def set_car_state(car_state):
    ...
    rospy.loginfo("Car in " + (
        "AUTO" if car_state == CarState.AUTO else "MANUAL") + " mode.")
    publish()
